...
async def test_registration(
    ...
    with patch("routes.accounts.send_activation_email_task.delay") as mock_delay:
    ...
    email_html = f"<strong id='email'>{user_data['email']}</strong><a id='link' href='http://testserver/activate/{response_data['id']}'>Activate</a>"
    soup = BeautifulSoup(email_html, "html.parser")
    email_element = soup.find("strong", id="email")
    assert email_element is not None
    validate_email(email_element.text)
    assert email_element.text == user_data["email"]
    link_element = soup.find("a", id="link")
    assert link_element is not None
    activation_url = link_element["href"]
    assert activation_url.startswith("http")
    mock_delay.assert_called_once_with(user_data["email"], "http://127.0.0.1/accounts/activate/")
