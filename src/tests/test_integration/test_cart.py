...
async def test_cart_crud(client, jwt_manager, test_user, test_movie):
    ...
    add_response = await client.post("/api/v1/cart/items",
...
@pytest.mark.asyncio
async def test_delete_all_movies(client, jwt_manager, test_user, test_movie, test_movie2):
    ...
    await client.post("/api/v1/cart/items",
                      ...
                      headers=headers)
    await client.post("/api/v1/cart/items",
                      json={"movie_id": test_movie2.id},
                      headers=headers)
    cart_response_before_delete = await client.get("/api/v1/cart/me",
                                                   headers=headers)
    cart_data_before_delete = cart_response_before_delete.json()
    assert len(cart_data_before_delete) == 2

    response = await client.delete(f"/api/v1/cart/items/",
                                   headers=headers)
    assert response.status_code == 204

    response_after_delete = await client.get(f"/api/v1/cart/me",
                                             headers=headers)
...
async def test_delete_movies_from_empty_cart(client, jwt_manager, test_user):
    ...
    headers = {"Authorization": f"Bearer {token}"}
    cart_response = await client.get("/api/v1/cart/me",
                                     headers=headers)
